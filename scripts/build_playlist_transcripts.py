...
def main() -> int:
    ...
    try:
        ...
        if not video_ids:
            data["error"] = "Не удалось извлечь видео из плейлиста"
        ...
        if not data["chunks"]:
            data["error"] = "Субтитры не извлечены из плейлиста (ограничение доступа или отсутствуют captions)"

        OUT_PATH.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
        return 0
    except Exception as exc:
        OUT_PATH.write_text(json.dumps(payload(f"Сбой сборки индекса: {exc}"), ensure_ascii=False, indent=2), encoding="utf-8")
        return 1
